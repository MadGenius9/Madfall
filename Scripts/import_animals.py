...
def main():
    ...
    for name in ANIMALS:
        ...
        # The pack's glTF files carry every clip twice, once as "<Clip>" and once as
        # "AnimalArmature|<Clip>" (the Blender action and its NLA copy); keep one.
        for asset_path in library.list_assets(destination, recursive=True):
            if "AnimalArmature_" in asset_path:
                library.delete_asset(asset_path.split(".")[0])
    ...
    unreal.log("[MadFall] Animals: {} imported, {} failed".format(len(ANIMALS) - failures, failures))

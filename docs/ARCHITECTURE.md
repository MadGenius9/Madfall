...
for a survivor rather than for the planner. A prefab that lists no biomes stands
in all of them, so the watchtower was everywhere and dominated the census;
subtract it and the desert had two buildings, both out of reach of a new
survivor (tier 2 and tier 4), the beach had none of its own, the forest had a
cabin and the bunker that is in five biomes, and the tiers went 1, 2, 4 - the
middle of a run had no POI a survivor had not already cleared. Tier is distance
from spawn, so "no tier 1 here" means the first hours of a game in that biome
are empty, and "no tier 3" means a stretch of the map with nothing new in it.

Five new prefabs, each aimed at one of those counts:

| | tier | biomes | fills |
|---|---|---|---|
| Way Station | 1 | desert, beach | the desert's empty first hours |
| Fishing Shack | 1 | beach | a coast with nothing of its own |
| Logging Camp | 2 | forest, tundra | a forest that was a cabin or the bunker |
| Collapsed Warehouse | 3 | plains, desert, tundra, forest | the missing tier |
| Wrecked Freighter | 3 | beach | a coast worth walking outward along |

They are generated by `Scripts/make_sample_prefabs.ps1`, which is where the
first four came from: a prefab's voxels are run-length encoded and nobody
should hand-edit run lengths, so each building is a `(x, y, z) -> palette
index` function that a reviewer can read. The trader outpost and mountain cache
remain hand-authored files the script does not own.

- **They were counted, not assumed.** `mad.poi.near <x> <y> 16` at nine points
  across a world is a census of what the planner actually places, and all five
  appear in numbers: of roughly 4,300 placements, the warehouse 548, the way
  station 245, the freighter 231, the shack 205, the camp 325. The freighter was
  the one at risk - a 27x13 footprint on a biome that is a strip - and it places.
- **`MadFall.WorldGen.PoiContent`** is the claim as a test. Markers name a loot
  table and a spawn group as plain strings, and nothing resolved them until a
  player opened the crate: a typo shipped an empty container and a POI with no
  zombies in it. It now checks every marker against the loaded loot and zombie
  definitions, and then the coverage argument itself - every land biome has a
  tier 1 POI, at least two that chose it by name, and something above tier 1 to
  walk to; every tier from 1 to 4 has a POI. It earned its place immediately by
  failing: with the shack added, every POI on the beach was tier 1, so the coast
  was somewhere to start and never somewhere to go. The freighter exists because
  that check failed.
- **Two were looked at, not just tested.** A shape function can satisfy every
  assertion and still be wrong to stand in. The warehouse's rubble was laid by
...
6. **RETUNED: terrain was gentle at scale.** Height spanned −9 … 62, visually
   ...
   **What mountains cost, and what paid for it.** With the same session
   ...
   - *And then it failed again, and the cause was another game.* The POI work
     failed the tail gate twice (1.55%, 1.87%) at 92.7 and 81.5 fps. An
     interleaved A/B - the five new prefab files moved aside and back between
     runs, so the machine's drift hit both sides - settled it: at matched frame
     rates (90.8 against 87.0 fps) the tail was 1.66% with them and 1.72%
     without. Then `Get-Process` found the answer: another game, 5.2 GB
     resident and nearly four CPU-hours, had been running the whole time. The
     same session measured 42 fps at its worst and 136 fps at its best on that
     box, against 105-136 when it was quiet.

     That is the end of the idea that any of these numbers can be read off a
     busy machine. The tail moves with the frame rate, the worst frame moves
     with it, and so does the mean - meshing is budgeted per frame, so fewer,
     fatter frames each do more MadFall work. **The stage now refuses to judge
     a session below 100 fps.** It retries up to four times, waiting a minute
     each; a session at or above the floor is measured as before (two measured
     failures is a regression); if no attempt gets there, it prints a warning
     naming the frame rate, says to check `Get-Process` and re-measure with
